...
#los nombres de las cabeceras del archivo csv para entender
#
# Columns:
# total_bill, tip, sex, smoker, day, time, size
# -> facilitar la consulta de nombres 

#una primera consulta en SQL
#
# SELECT *, tip/total_bill as tip_rate  >Seleccionatodos los campos y cree 
# FROM tips;
#
...
def mapper_query_1(sequence):
    """Mapper"""
    result = []
    for index, (_, row) in enumerate(sequence): #enumera cada elemento
        if index == 0: #la cabecera
            result.append(
                (
                    index,
                    row.strip() + ",tip_rate")
            ...
            row_values = row.strip().split(",") #elementos separados
